...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  IMU_AccelRead(&imu);
	  roll = atan2(imu.acc[1], imu.acc[2]);
	  roll *= 57.3;
	  pitch = atan2(imu.acc[0], imu.acc[2]);
	  pitch *= 57.3;
	  sprintf(sbuf, "%5.2f, ", roll);
	  HAL_UART_Transmit(&huart3, sbuf, 8, HAL_MAX_DELAY);
	  sprintf(sbuf, "%5.2f, ", pitch);
	  HAL_UART_Transmit(&huart3, sbuf, 8, HAL_MAX_DELAY);
	  ...
	  sprintf(sbuf, "%5.2f, ", gpitch);
	  HAL_UART_Transmit(&huart3, sbuf, 8, HAL_MAX_DELAY);
	  grow = grow + imu.gyro[0]*dt;
	  sprintf(sbuf, "%5.2f, ", grow);
	  HAL_UART_Transmit(&huart3, sbuf, 8, HAL_MAX_DELAY);
	  gyaw = gyaw + imu.gyro[2]*dt;
	  sprintf(sbuf, "%5.2f, ", gyaw);
	  HAL_UART_Transmit(&huart3, sbuf, 8, HAL_MAX_DELAY);
